...
1. **Uniqueness.** CPUX decompositions are unique — every intention path has exactly one normalization structure. This mirrors the uniqueness of **e** as the sole fixed point of differentiation.

2. **Cyclicality.** Intentions can generate sub-intentions that reference back, creating hierarchical execution meshes with cycles. The minimum structural complexity of a self-returning mesh is a candidate for an intrinsic geometric constant — Intention Space's analogue of **π**.

3. **Trivalence.** Pulse semantics are trivalent: Y (resolved), N (blocked), Unknown (neither resolved nor blocked). The Unknown state is the state of *unresolved identity* — the system has not yet returned to itself.
...
1. **Construct a minimal CPUX system with a resolution metric.** Define a distance function over intention paths (e.g., number of DN evaluations to reach O). Measure whether convergence to fixed-point intentions exhibits a characteristic constant. Compare this constant's properties to those of **e**.

2. **Enumerate minimal self-returning meshes.** In a finite CPUX system, find the smallest execution mesh that forms a complete cycle. Investigate whether the ratio of cyclic to acyclic mesh complexity converges to an invariant as system size grows. Compare this to **π**'s role as a ratio invariant (circumference to diameter).
...
4. **Category-theoretic formalisation.** Define Intention Space as a category where objects are intention states and morphisms are resolution steps. Define an endofunctor (resolution operator). Investigate its fixed points (**e**-analogues) and its periodic orbits (**π**-analogues). This would connect the informal intuitions to a rigorous mathematical framework.
...
1. Define a resolution metric for CPUX.
