...
When CPUX sends cpux:cell-update, GridLookout matches using:
    ...
    Level 1: icId matches subscription
    Level 2: intentionId matches subscription
    Level 3: pulsePhrase matches cell's pulsePhrase
...
Maintenance Rule: When adding new DNs:
    ...
    Register in cpux-engine.js DN_REGISTRY section
    Add to getRegisteredDNs() output for debugging
    Document expected input Pulses and output Pulses
...
10. Key Architectural Principles
    ...
    GridLookout is the native layer — It exists before any app, survives app crashes, and maintains the WebSocket bridge.
    CPUX is ephemeral — It can be restarted without losing GridLookout state. The FieldBoard can be reconstructed from persistence.
    Manifest is declarative — The app manifest describes what (ICs, cells, bindings), not how (execution order is determined by the Field).
    DNs are pure functions — A DN receives input Pulses and context, returns output Pulses. No side effects, no direct DOM access.
    All state flows through Pulses — Cells don't hold state; they reflect Pulse responses from the Field.
